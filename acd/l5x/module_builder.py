...
def _has_produced_connection(cur, object_id: int, short_header: bool) -> bool:
    """True if the module owns >= 1 live produced-tag connection.

    A produced connection is a live (dead_oids-filtered) child under the module's
    RxMapConnectionCollection whose ext-attr 0x01 blob leads with the fmt word 10.
    Used only by the modern-Local Communications rule.
    """
    try:
        rcc = cur.execute(
            "SELECT object_id FROM comps WHERE parent_id=? AND "
            "comp_name='RxMapConnectionCollection'", (object_id,)).fetchone()
        if not rcc:
            return False
        dead = CompsRecord.dead_oids(cur, short_header)
        for oid, rec in cur.execute(
                "SELECT object_id, record FROM comps WHERE parent_id=?",
                (rcc[0],)).fetchall():
            if rec is None or oid in dead:
                continue
            blob = CompsRecord.read_value_attrs(
                bytes(rec), short_header, body_mode=True).get(0x01, b"")
            if len(blob) >= 2 and struct.unpack_from("<H", blob, 0)[0] == 10:
                return True
        return False
    except Exception:
        return False
...
class Module(L5xElement):
    ...
    def to_xml(self) -> str:
        ...
        if _cw in (0x106, 0x609, 0x60A):
            _emit_comm = False
        elif _cw == 0x900:
            # A modern (class 0x900) Local controller emits a bare
            # <Communications> from firmware major >= 36 when its backplane is a
            # 5069 bus and it owns >= 1 live produced-tag (fmt-10) connection.
            # Version-keyed, same style as the ExternalAccess major-20 gate.
            _emit_comm = (self._sw_major >= 36
                          and 'Type="5069"' in ports
                          and self._local_produced_conn)
        elif _cw in (0x104, 0x109):
            _emit_comm = _nconn > 0 or self._comm_method is not None
        elif _cw in (0x111, 0x702, 0x800):
            # 0x111 (1756-EN2TR/EN3TR bridge) and 0x702 (5094-AENTR safety
            # adapter) carry Communications only with I/O, config, or a CommMethod;
            # a bare bridge/adapter (no connections, no <CF>) omits it, like 0x800.
